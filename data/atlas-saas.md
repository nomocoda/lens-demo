...
**Purpose:** This is the synthetic data source the Lens Demo uses to generate dynamic insight cards and ground chat conversations. It is **not** the persona document (that lives in `nomocoda/lens-voice`). It is the *fictional company state* — the "data" Lens is reading from in the demo, in lieu of real client systems.

**How it gets used:** The Cloudflare Worker reads this document and includes it in every Claude API call that generates insight cards or seeds a chat. The persona prompt tells Claude *who Lens is*. This document tells Claude *what Lens currently sees about Atlas SaaS*.
...
**Atlas SaaS, Inc.** — Mid-market B2B workflow-automation platform headquartered in Austin, Texas. Founded 2017, currently Series B ($28M raised to date). CEO and co-founder has led the company since inception.
...
2. Landing the Ridgeline Health deal — would be the company's first enterprise logo and proof of upmarket readiness
...
- Three senior engineers from FlowStack (competitor) updated their LinkedIn profiles to "open to work" last week — potential recruiting opportunity.
...
- **Clara Mendes** (yesterday, 4:47 PM, #engineering): *"Sprint 22 retro: velocity held. But I need to flag — the rewrite has zero test coverage on the auth module and we're 3 weeks from beta. Daniel, can we talk about this tomorrow?"*
...
- **Surface, don't decide.** The card should make the situation legible. The decision is the CEO's to make. The card can offer a recommended next step but should never demand action.
